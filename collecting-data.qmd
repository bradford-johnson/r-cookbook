...
Below I will show a simple script using the `rvest` package and `tidyverse` package that can scrape us some data from [Steam's](https://store.steampowered.com/stats/) game stats page. Steam is a video game distribution service, and we will scrape a couple columns from their live 'Top games by current player count' table.
